import { SlashCommandBuilder, ChatInputCommandInteraction } from "discord.js";
import { getPlayer, resolveSkill, formatNumber } from "../lib/wom.js";
import { getLinkedRsn } from "../lib/api.js";
import { clanEmbed, errorEmbed } from "../lib/embeds.js";

export default {
  data: new SlashCommandBuilder()
    .setName("lvl")
    .setDescription("Check a specific skill level")
    .addStringOption((opt) =>
      opt.setName("skill").setDescription("Skill name (e.g. slayer, wc, rc)").setRequired(true)
    )
    .addUserOption((opt) =>
      opt.setName("member").setDescription("Discord member to check").setRequired(false)
    ),

  async execute(interaction: ChatInputCommandInteraction) {
    await interaction.deferReply();

    const skillInput = interaction.options.getString("skill", true);
    const target = interaction.options.getUser("member") ?? interaction.user;
    const skill = resolveSkill(skillInput);

    const rsn = await getLinkedRsn(target.id);
    if (!rsn) {
      await interaction.editReply({
        embeds: [errorEmbed(`${target.id === interaction.user.id ? "You haven't" : `${target.displayName} hasn't`} linked their RSN.`)],
      });
      return;
    }

    const player = await getPlayer(rsn);
    if (!player?.latestSnapshot) {
      await interaction.editReply({ embeds: [errorEmbed(`Could not fetch data for "${rsn}".`)] });
      return;
    }

    const skillData = player.latestSnapshot.data?.skills?.[skill];
    if (!skillData) {
      await interaction.editReply({ embeds: [errorEmbed(`Unknown skill: "${skillInput}". Try the full name or abbreviation (wc, rc, fm, etc.).`)] });
      return;
    }

    const xpToNext = skill === "overall" ? null : getXpToNext(skillData.level, skillData.experience);
    const skillName = skill.charAt(0).toUpperCase() + skill.slice(1);

    const embed = clanEmbed()
      .setTitle(`${player.displayName} — ${skillName}`)
      .addFields(
        { name: "Level", value: `**${skillData.level}**`, inline: true },
        { name: "XP", value: formatNumber(skillData.experience), inline: true },
        { name: "Rank", value: skillData.rank > 0 ? `#${formatNumber(skillData.rank)}` : "Unranked", inline: true },
      );

    if (xpToNext !== null && skillData.level < 99) {
      embed.addFields({ name: "XP to Next Level", value: formatNumber(xpToNext), inline: true });
    }

    if (skillData.level >= 99) {
      embed.setDescription("🎉 **99!** Maxed!");
    }

    await interaction.editReply({ embeds: [embed] });
  },
};

// XP table for levels 1-99
const XP_TABLE = [
  0, 0, 83, 174, 276, 388, 512, 650, 801, 969, 1154, 1358, 1584, 1833, 2107,
  2411, 2746, 3115, 3523, 3973, 4470, 5018, 5624, 6291, 7028, 7842, 8740,
  9730, 10824, 12031, 13363, 14833, 16456, 18247, 20224, 22406, 24815, 27473,
  30408, 33648, 37224, 41171, 45529, 50339, 55649, 61512, 67983, 75127, 83014,
  91721, 101333, 111945, 123660, 136594, 150872, 166636, 184040, 203254,
  224466, 247886, 273742, 302288, 333804, 368599, 407015, 449428, 496254,
  547953, 605032, 668051, 737627, 814445, 899257, 992895, 1096278, 1210421,
  1336443, 1475581, 1629200, 1798808, 1986068, 2192818, 2421087, 2673114,
  2951373, 3258594, 3597792, 3972294, 4385776, 4842295, 5346332, 5902831,
  6517253, 7195629, 7944614, 8771558, 9684577, 10692629, 11805606, 13034431,
];

function getXpToNext(level: number, xp: number): number {
  if (level >= 99) return 0;
  const nextXp = XP_TABLE[level + 1] ?? 0;
  return Math.max(0, nextXp - xp);
}
